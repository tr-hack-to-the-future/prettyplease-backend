...
public class SponsorRequestHandler implements com.amazonaws.services.lambda.runtime.RequestHandler<Map<String, Object>, ApiGatewayResponse> {

    private static final Logger LOG = LogManager.getLogger(SponsorRequestHandler.class);
    ...
    // requests excluding those already with an offer by the sponsor
    private static final String getRequestsBySponsorIdSql = "SELECT f.requestId, f.charityId, f.eventDescription, f.incentive, " +
            ...
            "ON c.charityId = f.charityId " +
            "WHERE f.requestId NOT IN (SELECT o.requestId FROM prettyplease.SponsorOffer o WHERE o.sponsorId = ?) " +
            "ORDER BY f.createdAt, c.name;";
    ...
    public ApiGatewayResponse handleRequest(Map<String, Object> input, Context context) {
        ...
        if ("GET".equalsIgnoreCase(httpMethod)) {
            try {
                ...
                response = getRequestsById(input, sponsorId);
    ...
    private List<CharityRequest> getRequestsById(Map<String, Object> input, String sponsorId) throws SQLException, ClassNotFoundException {
        List<CharityRequest> requests = new ArrayList<>();
        LOG.info("sql= " + getRequestsBySponsorIdSql);
        try (
                Connection connection = getDatabaseConnection();
                PreparedStatement preparedStatement = connection.prepareStatement(getRequestsBySponsorIdSql);
        ) {
