...
public class CharityRequestHandler implements com.amazonaws.services.lambda.runtime.RequestHandler<Map<String, Object>, ApiGatewayResponse> {

    private static final Logger LOG = LogManager.getLogger(CharityRequestHandler.class);
    ...
    private static final String getRequestsByIdSql = "SELECT f.requestId, f.charityId, f.eventDescription, f.incentive, " +
    ...
    private static final String getRequestsSql = "SELECT f.requestId, f.charityId, f.eventDescription, f.incentive, " +
            "f.amountRequested, f.amountAgreed, f.isSingleEvent, f.durationInYears, f.agreedDurationInYears, " +
            "f.requestStatus, f.requestDate, f.dueDate, f.createdAt, " +
            "c.name as charityName, c.description as charityDescription, " +
            "c.imageUrl as charityImgUrl, c.webUrl as charityWebUrl \n" +
            "FROM prettyplease.FundRequest f INNER JOIN prettyplease.Charity c\n" +
            "ON c.charityId = f.charityId ORDER BY f.createdAt, c.name";
        ...
        if ("GET".equalsIgnoreCase(httpMethod)) {
            try {
                if (input.get("pathParameters") == null) {
                    ...
                    response = getRequestsById(input, requestId);
                }
    ...
    private List<CharityRequest> getRequests(Map<String, Object> input) throws SQLException, ClassNotFoundException {
        ...
        try (
                ...
                PreparedStatement preparedStatement = connection.prepareStatement(getRequestsSql);
    ...
    private List<CharityRequest> getRequestsById(Map<String, Object> input, String requestId) throws SQLException, ClassNotFoundException {
        List<CharityRequest> requests = new ArrayList<>();
        try (
                Connection connection = getDatabaseConnection();
                PreparedStatement preparedStatement = connection.prepareStatement(getRequestsByIdSql);
        ) {
